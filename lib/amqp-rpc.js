...
class RPC {
  ...
  constructor(address) {
    this.ready = amqp.connect().then((connection) => {
      ...
      console.error('can\'t connect rabbitmq', err);
      process.exit(1);
    });
  }
  ...
  on(routingKey, handler) {
    this.ready.then(() => {
      ...
      ch.consume(routingKey, function(msg) {
        ...
        if (helper.isGenerator(handler)) {

          var task = Promise.coroutine(handler);
          task(requestParams).then(function(responseParams) {
